...
class SearchListView(generics.GenericAPIView):
    def get(self, request, *args, **kwargs):
        ...
        query = request.GET.get('q')    #這邊去拿 `${baseEndpoint}/search/?${searchParams}`中的q，也就是searchParams中的q，q在index.html中的search表單中有定義name="q"
        public = str(request.GET.get('public')) != "0"   #這邊去拿 `${baseEndpoint}/search/?${searchParams}`中的public，也就是searchParams中的public，但是目前insex.html搜尋表單內我並沒有添加public這個參數所以目前?${searchParams}中沒有存取到public這個參數
